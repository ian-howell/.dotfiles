...
- `pi -p "<prompt>"` runs a single-shot print-mode session and exits.
- `pi -c` continues the most recent session; `pi -r` opens the resume picker.
- Slash commands (`/model`, `/settings`, `/resume`, `/tree`, `/compact`) run inside the TUI.
- Skills are invoked with `/skill:<name>`; prompt templates expand via `/<template-name>`.
