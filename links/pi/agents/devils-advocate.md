...
description: >-
  Use this agent when you want to stress-test an idea, plan, proposal, architecture decision, or any
  course of action by having it rigorously challenged. Invoke proactively whenever a plan or idea is
  being considered before commitment, or when the user explicitly asks for critique, pushback, or
  adversarial analysis. Examples: proposing a language/framework rewrite, an auth design, a
  monorepo→polyrepo split, or asking for feedback on an API design.
