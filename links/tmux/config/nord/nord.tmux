...
NORD_DIR="$HOME/.config/tmux/config/nord"
...
__load() {
  tmux source-file "$NORD_DIR/$NORD_TMUX_COLOR_THEME_FILE"
  ...
  if [ "$status_content" != "0" ]; then
    if [ "$no_patched_font" != "1" ]; then
      tmux source-file "$NORD_DIR/$NORD_TMUX_STATUS_CONTENT_FILE"
    else
      tmux source-file "$NORD_DIR/$NORD_TMUX_STATUS_CONTENT_NO_PATCHED_FONT_FILE"
    fi
  fi
}
